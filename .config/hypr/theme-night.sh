#!/bin/sh
# Hyprland night theme — standalone copy of the sway theme-night.sh.
# Only the compositor block (hyprctl) differs from the sway version.
hyprctl keyword misc:background_color       'rgb(1c1b16)'
hyprctl keyword general:col.active_border   'rgb(f2f1e5)'
hyprctl keyword general:col.inactive_border 'rgb(2e2d26)'

cp ~/.config/waybar/style-night.css ~/.config/waybar/style.css
pkill -SIGUSR2 waybar

# Colours only — foot.ini itself is hand-owned and must not be rewritten here,
# or the font size set in it is lost on every toggle.
cp ~/.config/foot/colors-night.ini ~/.config/foot/colors.ini

# System color scheme + Firefox live theme
gsettings set org.gnome.desktop.interface color-scheme 'prefer-dark'

# GTK apps (Thunar, file dialogs) — swap recolored gtk.css and force a live reload
cp ~/.config/gtk-3.0/gtk-night.css ~/.config/gtk-3.0/gtk.css
gsettings set org.gnome.desktop.interface icon-theme 'Papirus-Rice-Dark'
gsettings set org.gnome.desktop.interface gtk-theme ''
gsettings set org.gnome.desktop.interface gtk-theme 'Adwaita'

# Claude Code theme
sed -i 's/"theme": "light"/"theme": "dark"/' ~/.claude/settings.json

# Apply colors live to all running foot terminals
apply_term_colors() {
    printf '\033]10;#f2f1e5\007'  # foreground
    printf '\033]11;#1c1b16\007'  # background
    printf '\033]12;#f2f1e5\007'  # cursor
    printf '\033]4;0;#2e2d26\007'
    for i in 1 2 3 4 5 6; do printf '\033]4;%d;#bbbbaf\007' $i; done
    printf '\033]4;7;#f2f1e5\007'
    printf '\033]4;8;#3e3d34\007'
    for i in 9 10 11 12 13 14; do printf '\033]4;%d;#ddddd1\007' $i; done
    printf '\033]4;15;#f2f1e5\007'
}

for pid in $(pgrep -x foot); do
    for child in $(pgrep -P "$pid"); do
        tty=$(readlink /proc/"$child"/fd/0 2>/dev/null)
        case "$tty" in /dev/pts/*) apply_term_colors > "$tty" ;; esac
    done
done
